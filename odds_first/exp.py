...
def main():
    ...
    match args.prompt_style:
        case "base":
            with open(f"odds_first/prompts/of.txt", 'r') as file:
                prompt = file.read().strip()
        case "cot":
            with open(f"odds_first/prompts/of.cot.txt", 'r') as file:
                prompt = file.read().strip()
    for line in dataset:
