...
def main():
    ...
    match args.prompt_style:
        case "base":
            with open(f"missing_duplicate/prompts/md.txt", 'r') as file:
                prompt = file.read().strip()
        case "cot":
            with open(f"missing_duplicate/prompts/md.cot.txt", 'r') as file:
                prompt = file.read().strip()
    for line in dataset:
